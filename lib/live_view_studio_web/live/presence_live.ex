defmodule LiveViewStudioWeb.PresenceLive do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      socket
      |> assign(is_playing: false, presences: dedupe_presence_map(presences))
  ...
  def handle_event("toggle-playing", _, socket) do
    ...
    # Ensure other users can see that our playing state has changed, this will broadcast
    # to the handle_info for the presence_diff
    %{current_user: current_user} = socket.assigns
  ...
  defp add_presences(socket, joins) do
    presences = Map.merge(socket.assigns.presences, dedupe_presence_map(joins))
    assign(socket, :presences, presences)
  end
end
